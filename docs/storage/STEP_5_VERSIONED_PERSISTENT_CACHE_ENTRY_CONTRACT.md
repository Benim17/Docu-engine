...
Status: **DESIGN LOCKED — approved for Step 5A implementation**  
Branch: `feature/cache-storage-foundation`  
Scope: **Persistent cache-entry design contract; Step 5A may implement pure models, validation, serialization, and path derivation only**
...
## 23. Locked clarifications from design review

The following decisions remove implementation ambiguity and are normative for contract v1.

### 23.1 Canonical JSON encoding

Whenever contract v1 requires deterministic JSON bytes or a digest of a JSON document, the bytes must be produced using all of these rules:

- UTF-8 encoding without BOM;
- object keys sorted lexicographically by Unicode code point;
- separators exactly `,` and `:` with no insignificant whitespace;
- no trailing newline in the digested byte representation;
- JSON strings escaped according to the JSON standard;
- integers only for contract numeric fields; floating-point values are forbidden in identity-bearing or digested contract documents;
- duplicate object keys are rejected during parsing;
- non-finite numbers are rejected;
- Unicode strings are preserved as supplied after field-specific validation; no implicit filesystem-dependent normalization is permitted.

Writers may pretty-print diagnostic copies elsewhere, but `metadata.json`, `manifest.json`, and `COMPLETE` in a contract-v1 entry must use the canonical encoding above.

### 23.2 Unknown-field policy

Contract-v1 readers use a strict schema:

- unknown fields in `metadata.json`, `manifest.json`, `COMPLETE`, namespace objects, manifest records, and lock metadata are rejected;
- missing optional fields are allowed only when the schema explicitly marks them optional;
- forward evolution requires a new relevant schema or contract version rather than silently ignoring fields.

This intentionally favors integrity and reproducibility over permissive forward parsing.

### 23.3 Namespace canonical form

The namespace has exactly three logical components:
...
- `domain` and `producer_id` use the segment validation in section 4.1;
- `producer_schema_version` is a positive base-10 integer with no sign and no leading zeroes;
- the canonical path form is the three components joined by `/`;
- callers must construct a validated `CacheNamespace`; raw namespace strings must never be interpolated directly into paths;
- the namespace producer ID and schema version must exactly match the corresponding metadata producer fields.

### 23.4 Cache-key storage and verification

`entry_digest` is always computed from `CacheKey.canonical_bytes()` and remains the sole cache-key-derived physical path component.

The metadata cache-key object stores:

- `canonical_version`;
- a lossless canonical textual representation only if the existing `CacheKey` contract provides one;
- otherwise, an algorithm-qualified digest of the canonical bytes.

Step 5A must reuse the existing validated `CacheKey` API and must not invent a second canonicalization algorithm. Raw input material, physical paths, secrets, or machine-local observations must not be copied into cache metadata merely to aid debugging.

### 23.5 File-name and manifest path policy

Manifest `relative_path` values are relative to `payload/` and use `/` as the only separator regardless of host operating system.
...
- no empty, `.` or `..` segment;
- no segment ending in a space or dot;
- no path that aliases another manifest path after the implementation's declared portable comparison rule;
- deterministic ordinal sorting by canonical relative-path string.

Contract v1 adopts a conservative portable comparison rule: paths that differ only by ASCII case are rejected within the same manifest. This avoids creating entries that become ambiguous when moved between common case-sensitive and case-insensitive filesystems.

### 23.6 Hardlink policy

Contract v1 payload construction and validation reject payload files with a detectable link count greater than one. A writer must not intentionally create payload hardlinks. Readers classify a detectable hardlinked payload as `INTEGRITY_FAILURE`.

This does not claim that every remote or unusual filesystem exposes reliable link-count information. Where hardlink status cannot be determined, the validator records that limitation but must still perform all other integrity checks.

### 23.7 Top-level entry contents

A valid final contract-v1 entry contains exactly:

- regular file `metadata.json`;
- regular file `manifest.json`;
- directory `payload`;
- regular file `COMPLETE`.

No additional top-level object is allowed. Empty payload directories are allowed only if the producer contract explicitly permits a zero-file payload and metadata/manifest counts are both zero.

### 23.8 Lookup and lock interaction

A lock does not invalidate an already valid immutable final entry.

Normative behavior:

- valid final entry present: return `HIT`, even if a stale or superfluous lock exists;
- final entry absent and matching active lock present: return `LOCKED_OR_IN_PROGRESS`;
- final entry absent and no matching lock present: return `MISS`;
- final entry present but invalid: return the most specific validation failure; the lock does not hide corruption;
- lookup never creates, refreshes, removes, or breaks a lock.

### 23.9 Result-state precedence

When more than one lookup problem exists, use this precedence:
...
5. `PRODUCER_MISMATCH`
6. `SCHEMA_MISMATCH`
7. `RUNTIME_FINGERPRINT_MISMATCH`
8. `LOCKED_OR_IN_PROGRESS`
9. `MISS`
10. `HIT`

A concrete implementation may attach multiple diagnostics, but the primary status must follow this order.

### 23.10 Runtime fingerprint comparison

Step 5A models the fingerprint and its declared comparison policy but does not infer compatibility.

The default contract-v1 comparison is exact equality of:

- fingerprint schema version;
- canonical key set;
- canonical values.

Any looser compatibility rule must be supplied explicitly by the producer adapter and tested independently. Unknown fingerprint schema versions are incompatible.

### 23.11 Atomic promotion boundary

Contract v1 defines promotion as an atomic rename of the complete staging directory into an absent final-entry path on the same filesystem.

It does not permit:

- copy-then-delete promotion;
- merging directories;
- replacement of an existing final entry;
- platform-specific fallback that weakens atomicity without a new reviewed contract decision.

Step 5A only models and derives the paths. It performs no rename or other filesystem mutation.

### 23.12 Step 5A module boundary

Step 5A may add pure modules corresponding to these responsibilities:

- version constants;
- `CacheNamespace` validation and canonical serialization;
- cache-entry metadata models;
- payload manifest models;
- completeness-marker models;
- lookup expectation/result/status models;
- deterministic JSON encoding helpers scoped to this contract;
- validated path derivation from an existing validated `CacheKey` plus approved cache root.

Step 5A must not add:
...
- promotion;
- recovery mutation;
- cleanup, deletion, eviction, or repair;
- a persistent index;
- changes that make Inventory or its CLI mutable.

---

## 24. Design review disposition

**Decision: APPROVED WITH LOCKED CLARIFICATIONS**

All Foundation Review conditions are represented in the design. Sections 23.1–23.12 are normative and take precedence over earlier wording where an ambiguity exists.

The approved next action is **Step 5A — pure contracts and tests only**. Production filesystem mutation remains prohibited until later phases receive separate review.
